...
class DatabaseManager:
    ...
    # novos métodos de vínculo
    def vincular_pai_aluno(self, aluno_id, pai_username):
        self.cursor.execute(
            "INSERT OR IGNORE INTO alunos_pais (aluno_id, pai_id) VALUES (?, ?)",
...
class SISPE:
    ...
    def criar_tela_vinculo(self):
        frame_vinculo = ttk.Frame(self.principal, padding="50 30 50 30", relief="groove")
    ...
    # Botão para voltar
        ttk.Button(frame_vinculo, text="Voltar", command=lambda: self.mostrar_frame("principal")).pack(pady=10)
