...
class PsicologoScreen(QWidget):
    ...
    def filtrar_alunos(self):
        if not self.app.usuario_logado:
            return  # 🔥 evita erro antes do login

        texto = self.inputBusca.text().strip().lower()
        alunos = self.db.listar_alunos(self.app.usuario_logado["id"])
        if texto:
    ...
    def obter_ultima_data_relatorio(self, aluno_id):
        ...
        if relatorios:
            data_str = relatorios[0][1]

            try:
                data = datetime.strptime(data_str, "%d/%m/%Y %H:%M %H:%M:%S")
    ...
    def cadastrar_aluno(self):
        ...
        if not nome or not sala or not serie:
            ...
            return
        if self.db.aluno_existe(
            nome, sala, serie, self.app.usuario_logado["id"]
        ):
            QMessageBox.warning(self, "Erro", "Aluno já cadastrado")
            return

        self.db.adicionar_aluno(
            nome, sala, serie, gravidade, self.app.usuario_logado["id"]
        )
